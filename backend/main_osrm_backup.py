...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("routivity_debug.log"), logging.StreamHandler()],
)
logging.getLogger("watchfiles").setLevel(logging.WARNING) # Silence file watcher logs
...
class TripRequest(BaseModel):
    ...
    user_preferences: Optional[UserPreferences] = None # Allow client to override/provide preferences directly
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            if e.response.status_code == 504:
                logger.warning(f"Overpass API timeout (504) for query='{query}'. Returning empty.")
            else:
                logger.error(f"Overpass API error {e.response.status_code}: {e}")
            return []
        except Exception as e:
            logger.error(f"Error fetching places from Overpass: {e}")
            return []
...
async def find_tourist_spots(checkpoints: List[Dict], max_detour: int, limit: int = 10) -> List[PlaceSuggestion]:
    """
    Find tourist spots along the route checkpoints.
    """
    # Sample checkpoints to avoid too many queries (e.g., every 10th checkpoint)
    # or just use a few key points (start, middle, end) if checkpoints are dense.
    # For now, let's pick 3 points distributed along the route + 2 random ones
    if not checkpoints:
        return []

    logger.info("Searching for tourist spots along the route...")
    
    # Select search points: 25%, 50%, 75% of the trip
    indices = [int(len(checkpoints) * 0.25), int(len(checkpoints) * 0.5), int(len(checkpoints) * 0.75)]
    search_points = [checkpoints[i] for i in indices if i < len(checkpoints)]
    
    found_places = {} # Map osm_id -> PlaceSuggestion

    for point in search_points:
        # Search radius 10km for attractions
        q = f"""
        [out:json][timeout:25];
        (
          node["tourism"~"attraction|viewpoint|museum|zoo|theme_park"](around:10000,{point['lat']},{point['lng']});
          way["tourism"~"attraction|viewpoint|museum|zoo|theme_park"](around:10000,{point['lat']},{point['lng']});
          relation["tourism"~"attraction|viewpoint|museum|zoo|theme_park"](around:10000,{point['lat']},{point['lng']});
          node["historic"~"monument|memorial|castle|ruins"](around:10000,{point['lat']},{point['lng']});
          way["historic"~"monument|memorial|castle|ruins"](around:10000,{point['lat']},{point['lng']});
        );
        out center;
        """
        try:
            ...
            for el in elements:
                ...
                # Basic PlaceSuggestion without detour/eta yet
                found_places[osm_id] = PlaceSuggestion(
                    osm_id=osm_id,
                    name=name,
                    location=LatLng(lat=float(lat), lng=float(lon)),
                    detour_minutes=0,  # Calculated later
                    eta_iso="",        # Calculated later
                    tags=tags,
                    personalization_score=4.0, # Default high score for attractions
                    match_reasons=["Tourist Attraction"]
                )
        except Exception as e:
            logger.error(f"Error fetching tourist spots: {e}")

    # Convert to list
    candidates = list(found_places.values())
    logger.info(f"Found {len(candidates)} potential tourist spots")
...
async def create_trip(tr: TripRequest):
    ...
    try:
        ...
        # 1. Fetch user preferences if customized in request, or user_id provided
        user_prefs = None
        personalization_used = False
        
        if tr.user_preferences:
            user_prefs = tr.user_preferences
            personalization_used = True
            logger.info(f"Using explicitly passed personalization preferences: {user_prefs}")
        elif tr.user_id:
            user_prefs = await get_user_preferences(tr.user_id)
            if user_prefs:
                personalization_used = True
                logger.info(f"Using fetched personalization for user {tr.user_id}: {user_prefs}")
        ...
        if user_prefs:
            ...
            logger.info("No preferences found/provided, using default request parameters")
...
if __name__ == "__main__":
    uvicorn.run("main_osrm:app", host="0.0.0.0", port=8000, reload=False, reload_excludes=["*.log"])
